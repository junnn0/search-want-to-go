...
#### DB 샤딩
DB 샤딩을 통해 DB에 집중되는 부하를 줄입니다. 샤딩 키는 각 리소스별로 유니크하고 일정한 비율을 가지고 생성되는 ID 값을 사용합니다.

#### READ 서버 분리
검색 서비스에 들어오는 대부분의 트래픽은 조회와 관련된 트래픽이므로 조회 명령만을 처리하는 API 서버군을 따로 두어 관리하는 방법도 있습니다.<br/>
코드는 분리해도 되고 하나의 코드나 이미지를 사용한다면 API 서버에서 조회에 사용되는 Bean과 변경에 사용되는 Bean을 Profile로 분리하여 실행 시 활성화할 Profile을 다르게 넘겨주면 가능합니다.   

#### 캐시 서버 적극 사용
마찬가지로 검색 서비스의 부하는 대부분이 조회 명령입니다. 조회 명령을 효과적으로 처리하기 위해서 Redis와 같은 In-memory DB를 캐시 서버로 적극 사용하는 방법도 있습니다.
