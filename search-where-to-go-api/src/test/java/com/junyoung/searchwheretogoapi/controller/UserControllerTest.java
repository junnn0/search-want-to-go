...
class UserControllerTest {
  ...
  void test_get_user_success() throws Exception {
    mockMvc
        .perform(get("/user").header(HttpHeaders.AUTHORIZATION, TestDataUtil.createToken()))
  ...
  void test_get_user_with_empty_user() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.header.resultCode").value(ResponseType.NOT_LOGIN_USER.getCode()))
        .andExpect(
    ...
    mockMvc
        .perform(
            post("/users")
                ...
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.header.isSuccessful").value(false))
        .andExpect(jsonPath("$.header.resultCode").value(ResponseType.INVALID_PARAMETER.getCode()));
  }
  ...
  void test_login_user_incorrect_password() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.header.isSuccessful").value(false))
        .andExpect(jsonPath("$.header.resultCode").value(ResponseType.NOT_EXISTS_USER.getCode()));
  }
}
